...
<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 md:py-14">
    {{-- 页面标题与搜索 --}}
    <div class="mb-10">
        <h1 class="text-2xl md:text-3xl font-serif font-semibold text-primary-800 mb-2">文章列表</h1>
        <p class="text-dark-800/60 text-sm mb-6">人间清醒，治愈文字</p>
        <form method="GET" class="flex flex-wrap gap-3 p-4 bg-white rounded-2xl border border-haze-200 shadow-sm">
            <input type="text" name="keyword" value="{{ $keyword ?? '' }}" placeholder="搜索文章..." class="flex-1 min-w-[200px] rounded-xl border border-haze-200 px-4 py-2.5 focus:ring-2 focus:ring-primary-500/50 focus:border-primary-400 outline-none transition bg-haze-50/50">
            <select name="category_id" class="rounded-xl border border-haze-200 px-4 py-2.5 focus:ring-2 focus:ring-primary-500/50 outline-none bg-white">
                <option value="">全部分类</option>
                @foreach($categories as $c)
                <option value="{{ $c->id }}" {{ request('category_id') == $c->id ? 'selected' : '' }}>{{ $c->name }}</option>
                @endforeach
            </select>
            <select name="order" class="rounded-xl border border-haze-200 px-4 py-2.5 focus:ring-2 focus:ring-primary-500/50 outline-none bg-white">
            ...
            <div class="space-y-4">
                ...
                <a href="{{ route('front.articles.show', $a) }}" class="flex gap-4 md:gap-6 p-5 bg-white rounded-2xl border border-haze-200 hover:shadow-lg hover:shadow-primary-500/5 hover:border-primary-200 transition-all duration-300 block group">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-dark-800/60 text-sm mt-1">{{ $a->category?->name ?? '未分类' }} · {{ $a->created_at->format('Y-m-d') }} · 阅读 {{ $a->click_num ?? 0 }}</p>
                    </div>
                ...
                @empty
                <div class="py-20 text-center text-dark-800/50 bg-haze-50 rounded-2xl border border-haze-200">暂无文章，敬请期待</div>
                @endforelse
            </div>
