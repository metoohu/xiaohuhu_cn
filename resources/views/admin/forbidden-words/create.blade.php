...
@section('title', '新增违禁词 - ' . \App\Models\Setting::adminName())

@section('content')
<div class="bg-white rounded-lg shadow p-4 max-w-lg">
    <h2 class="text-xl font-bold mb-4">新增违禁词</h2>
    <form method="POST" action="{{ route('admin.forbidden-words.store') }}">
        @csrf
        @include('admin.forbidden-words._form', ['categories' => $categories])
