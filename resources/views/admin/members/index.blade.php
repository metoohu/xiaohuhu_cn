...
<div class="bg-white rounded-lg shadow p-4">
    ...
    {{-- 筛选 + 批量固定单行不换行；窄屏横向滚动 --}}
    <div class="mb-4 overflow-x-auto rounded-lg border border-slate-200 bg-slate-50/80 p-3 sm:p-4 [-webkit-overflow-scrolling:touch]">
        <div class="flex flex-nowrap items-center gap-x-3 gap-y-0">
            <form method="GET" class="flex shrink-0 flex-nowrap items-center gap-2">
                <input type="text" name="keyword" value="{{ request('keyword') }}" placeholder="关键词：昵称或邮箱" aria-label="关键词（昵称或邮箱）" class="h-9 w-44 shrink-0 rounded-md border border-slate-300 px-3 text-sm sm:w-52">
                <select name="banned" aria-label="评论状态" class="h-9 shrink-0 rounded-md border border-slate-300 bg-white px-2 text-sm">
            ...
            @if($members->total() > 0)
            <div class="h-8 w-px shrink-0 bg-slate-200" aria-hidden="true"></div>

            <div class="flex shrink-0 flex-nowrap items-center gap-2">
                <span class="shrink-0 text-sm whitespace-nowrap text-slate-600">已选 <strong id="member-selected-count" class="tabular-nums text-slate-900">0</strong></span>
                <div class="flex flex-nowrap items-center gap-1.5">
                    <form id="form-batch-mute" action="{{ route('admin.members.batch-mute') }}" method="POST" class="flex shrink-0 flex-nowrap items-center gap-1.5" onsubmit="return memberBatchPrepareSubmit(this, 'mute');">
                        @csrf
                        <input type="text" name="comment_ban_reason" maxlength="500" placeholder="禁言原因（选填）" aria-label="批量禁言原因" class="h-9 w-40 shrink-0 rounded-md border border-slate-300 px-2 text-sm sm:w-48">
                        <button type="submit" class="h-9 shrink-0 rounded-md bg-red-600 px-2.5 text-sm font-medium text-white hover:bg-red-700 sm:px-3">批量禁言</button>
                    </form>
                    <form id="form-batch-unmute" action="{{ route('admin.members.batch-unmute') }}" method="POST" class="flex shrink-0 items-center" onsubmit="return memberBatchPrepareSubmit(this, 'unmute');">
                        @csrf
                        <button type="submit" class="h-9 shrink-0 rounded-md bg-green-600 px-2.5 text-sm font-medium text-white hover:bg-green-700 whitespace-nowrap sm:px-3">批量解除禁言</button>
                    </form>
                </div>
            </div>
            @endif
