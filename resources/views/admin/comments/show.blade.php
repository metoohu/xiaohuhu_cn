...
<div class="bg-white rounded-lg shadow p-4">
    ...
    <dl class="grid grid-cols-1 sm:grid-cols-2 gap-2">
        ...
        <dt class="text-slate-500">文章 / 社区稿</dt>
        <dd>
            @if($comment->article)
                <a href="{{ route('admin.articles.edit', $comment->article) }}" class="text-blue-600">{{ $comment->article->title }}</a>
            @elseif($comment->userArticle)
                <a href="{{ route('admin.user-articles.show', $comment->userArticle) }}" class="text-blue-600">{{ $comment->userArticle->title }}</a>
            @else
                -
            @endif
        </dd>
